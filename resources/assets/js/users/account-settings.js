...
window.userProfile = FilePond.create(document.querySelector(".filepond"), {
    ...
    for (var i = 0; i < getDropdownElement.length; i++) {
        getDropdownElement[i].addEventListener("click", function () {
            ...
            if (dataValue === null && dataImage === null) {
                console.warn(
                    "No attributes are defined. Kindly define one attribute atleast"
                );
            }

            if (dataValue != "" && dataValue != null) {
                this.parentElement.parentNode.querySelector(
                    ".dropdown-toggle > .selectable-text"
                ).innerText = dataValue;
            }

            if (dataImage != "" && dataImage != null) {
                this.parentElement.parentNode
                    .querySelector(".dropdown-toggle > img")
                    .setAttribute("src", dataImage);
            }
        });
    }
}
...
selectableDropdown(
    document.querySelectorAll(".cardName-select .dropdown-item")
);
