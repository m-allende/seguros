    ...
    <x-slot:footerFiles>
        <script>
            $(document).on('click', '.btn-add', function() {
                $('.modal').modal("show")
                $('.form').trigger('reset')
                $('.modal').find('.modal-title').text('Agregar nueva Marca')
                $('.btn-save').show();
                $('.btn-update').hide()
            })
            ...
            $(document).ready(function() {
                ...
                btnUpdate.click(function() {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/config/branchvehicle/" + updateId,
                        data: formData,
                        success: function(data) {
                            if (data.status == 200) {
                                table.draw();
                    ...
                    Swal.fire({
                        ...
                        if (result.value) {
                            $.ajax({
                                type: "POST",
                                dataType: 'JSON',
                                url: "/config/branchvehicle/" + rowid,
                                data: {
                                    _method: 'delete',
                                    _token: token
                                },
                ...
                $(document).on("click", '.export-excel',
                    function(event) {
                        ...
                        $.ajax({
                            type: "GET",
                            url: "{{ route('branchvehicle.export') }}",
                ...
                $(document).on("click", '.btn-activate',
                    function(event) {
                        ...
                        Swal.fire({
                            title: "¿Desea dejar Vigente esta Marca?",
                            icon: "warning",
                            showCancelButton: true,
                            confirmButtonText: "Si",
                            cancelButtonText: "No",
                            ...
                            if (result.value) {
                                $.ajax({
                                    type: "POST",
                                    url: "/config/branchvehicle/activate",
                                    data: "id=" + id,
                                    success: function(datos) {
                                        table.draw();
                                    },
