    ...
    <x-slot:footerFiles>
        <script>
            $(document).on('click', '.btn-add', function() {
                $('.modal').modal("show")
                $('.form').trigger('reset')
                $('.modal').find('.modal-title').text('Agregar nuevo Uso')
                $('.btn-save').show();
                $('.btn-update').hide()
            })
            ...
            $(document).ready(function() {
                ...
                $(document).on('click', '.btn-edit', function() {
                    ...
                    modal.find('.modal-title').text('Modificar Uso')
                ...
                btnUpdate.click(function() {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/config/usevehicle/" + updateId,
                        data: formData,
                        success: function(data) {
                            if (data.status == 200) {
                                table.draw();
                ...
                $(document).on("click", '.btn-activate',
                    function(event) {
                        ...
                        Swal.fire({
                            title: "¿Desea dejar Vigente este Uso?",
                            icon: "warning",
                            showCancelButton: true,
                            confirmButtonText: "Si",
                            cancelButtonText: "No",
                            ...
                            if (result.value) {
                                $.ajax({
                                    type: "POST",
                                    url: "/config/usevehicle/activate",
                                    data: "id=" + id,
                                    success: function(datos) {
                                        table.draw();
                                    },
