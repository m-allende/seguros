                    ...
                    <div class="modal-content modal-content-light">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="branch_vehicle_id">Marca</label><br>
                                <select name="branch_vehicle_id" id="branch_vehicle_id"
    ...
    <x-slot:footerFiles>
        <script>
            $(document).on('click', '.btn-add', function() {
                $('.modal').modal("show")
                $('.form').trigger('reset')
                $('.modal').find('.modal-title').text('Agregar nuevo Modelo')
                $('.btn-save').show();
                $('.btn-update').hide()
            })
            ...
            $(document).ready(function() {
                ...
                $(document).on('click', '.btn-edit', function() {
                    ...
                    modal.find('.modal-title').text('Modificar Modelo')
                    ...
                    var newOption = new Option(rowData.branch.name, rowData.branch.id, true, true);
                    $('.select2-branch-modal').append(newOption).trigger('change');
                ...
                btnUpdate.click(function() {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/config/modelvehicle/" + updateId,
                        data: formData,
                        success: function(data) {
                            if (data.status == 200) {
                                table.draw();
                ...
                $(document).on("click", '.btn-activate',
                    function(event) {
                        ...
                        Swal.fire({
                            title: "¿Desea dejar Vigente este Modelo?",
                            icon: "warning",
                            showCancelButton: true,
                            confirmButtonText: "Si",
                            cancelButtonText: "No",
                            ...
                            if (result.value) {
                                $.ajax({
                                    type: "POST",
                                    url: "/config/modelvehicle/activate",
                                    data: "id=" + id,
                                    success: function(datos) {
                                        table.draw();
                                    },
